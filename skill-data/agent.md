...
6. If `zero graph status <input>` reports repository graph sync as enabled, run `zero graph verify-sync <input>` before build/test gates. Use `zero graph sync --from-source <input>` to refresh `zero.graph` from reviewed source changes and exact local `.0` projection bytes, or `zero graph sync --from-graph <input>` to refresh stale `.0` source projections from `zero.graph`.

...
void z_append_targets_json(ZBuf *buf) {
  ...
  for (size_t i = 0; i < target_count; i++) {
    ...
    zbuf_append(buf, ", \"backendFamilies\": {\"default\": \"direct\", \"known\": [\"direct\", \"llvm\"], \"available\": [\"direct\", \"llvm\"], \"fallbackPolicy\": \"none\", \"llvm\": {\"status\": \"ir-only\", \"buildable\": false, \"emit\": [\"llvm-ir\"], \"reason\": \"LLVM textual IR emission is available; native LLVM artifacts are not wired yet\"}}");
